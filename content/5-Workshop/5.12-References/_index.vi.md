---
title: "5.12. Tài liệu tham khảo"
date: 2024-01-01
weight: 12
chapter: false
---

# TÀI LIỆU THAM KHẢO

Trang này tổng hợp các tài liệu tham khảo được sử dụng trong workshop **Resume Fit**.

## 1. Source code and demo

* **1.1 Link demo Resume Fit:** [http://resumematching-alb-1223673352.us-east-1.elb.amazonaws.com](http://resumematching-alb-1223673352.us-east-1.elb.amazonaws.com)
* **1.2 Link source code Resume Fit:** [https://github.com/thuanthien-tran/resume-fit](https://github.com/thuanthien-tran/resume-fit)

## 2. AWS services used by the project

### Các chapter trong dự án

* [5.3 Networking](vi/5-workshop/5.3-networking/)
* [5.4 Security & Access](vi/5-workshop/5.4-security-access/)
* [5.5 Storage & Queue](vi/5-workshop/5.5-storage-queue/)
* [5.6 RDS Setup](vi/5-workshop/5.6-rds-setup/)
* [5.7 Compute Setup](vi/5-workshop/5.7-compute-setup/)
* [5.8 Scaling with ALB](vi/5-workshop/5.8-scaling-alb/)
* [5.9 Monitoring](vi/5-workshop/5.9-monitoring/)

### Tài liệu AWS

* [Amazon VPC documentation](https://docs.aws.amazon.com/vpc/)
* [Amazon EC2 documentation](https://docs.aws.amazon.com/ec2/)
* [Elastic Load Balancing - Application Load Balancers](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/)
* [Amazon EC2 Auto Scaling documentation](https://docs.aws.amazon.com/autoscaling/ec2/)
* [Amazon RDS for PostgreSQL documentation](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_PostgreSQL.html)
* [Amazon S3 documentation](https://docs.aws.amazon.com/s3/)
* [Amazon SQS documentation](https://docs.aws.amazon.com/sqs/)
* [AWS Secrets Manager documentation](https://docs.aws.amazon.com/secretsmanager/)
* [Amazon CloudWatch documentation](https://docs.aws.amazon.com/cloudwatch/)
* [AWS Identity and Access Management documentation](https://docs.aws.amazon.com/iam/)
* [AWS Systems Manager Session Manager documentation](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager.html)

## 3. Development and deployment

### Các chapter trong dự án

* [5.1 Introduction](vi/5-workshop/5.1-introduction/)
* [5.2 Prerequisites](vi/5-workshop/5.2-prerequisites/)
* [5.7.1 Create EC2 Instance](vi/5-workshop/5.7-compute-setup/5.7.1-create-ec2-instance/)
* [5.7.2 Shell Access and Software Setup](vi/5-workshop/5.7-compute-setup/5.7.2-connect-install-software/)
* [5.8.1 AMI Creation and Launch Template Setup](vi/5-workshop/5.8-scaling-alb/5.8.1-configure-ami-launch-template/)
* [5.8.2 Auto Scaling Group Setup](vi/5-workshop/5.8-scaling-alb/5.8.2-auto-scaling-group/)
* [5.8.3 Target Group and ALB Setup](vi/5-workshop/5.8-scaling-alb/5.8.3-target-group-alb/)
* [5.10 Testing](vi/5-workshop/5.10-testing/)

### Tài liệu tham khảo ngoài

* [AWS Well-Architected Framework](https://docs.aws.amazon.com/wellarchitected/latest/framework/welcome.html)
* [AWS Architecture Center](https://aws.amazon.com/architecture/)
* [Docker documentation](https://docs.docker.com/)
* [Git documentation](https://git-scm.com/doc)
* [GitHub Docs](https://docs.github.com/)
* [AWS CLI Command Reference](https://docs.aws.amazon.com/cli/latest/reference/)
* [Deploying web applications on Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/concepts.html)

## 4. Cost and resource cleanup

### Các chapter trong dự án

* [5.11 Clean Up & Cost](vi/5-workshop/5.11-cleanup/)

### Tài liệu tham khảo ngoài

* [AWS Pricing Calculator](https://calculator.aws/)
* [AWS Cost Management documentation](https://docs.aws.amazon.com/cost-management/)
* [AWS Billing and Cost Management User Guide](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/)
* [Amazon EC2: Terminate instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/terminating-instances.html)
* [Elastic Load Balancing: Delete a load balancer](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-delete.html)
* [Amazon RDS: Deleting a DB instance](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_DeleteInstance.html)
* [Amazon S3: Deleting a bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/delete-bucket.html)
* [Amazon VPC: Delete your VPC](https://docs.aws.amazon.com/vpc/latest/userguide/delete-vpc.html)
